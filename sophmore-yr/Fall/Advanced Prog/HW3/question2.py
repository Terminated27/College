...
class BinarySearchTree:
    ...
    def lowestCommonRoot(self, element_1, element_2):
        if self is None or self == element_1 or self == element_2:
            return self
        
        left = self.lowestCommonRoot(left, element_1, element_2)
        right = self.lowestCommonRoot(right)
    def _find_path(self, element_1, element_2, nature):
        ...
        self.steps += 1
        #find all successors in between

        #find root from element_1
        e1list = []
        e2list = []
        temp = element_1
        while temp.isRoot() == False:
            e1list.append(temp.key)
            temp = temp.parent
        e1list.append(temp.key)
        temp = element_2
        while temp.isRoot() == False:
            e2list.append(temp.key)
            temp = temp.parent
        e2list.append(temp.key)
        e1list.sort()
        e2list.sort()
        ...
        if nature == 0:
            ...
            e2.reverse()
            list_path = e1 + e2
        self.steps = len(list_path)# if i == j else 0
