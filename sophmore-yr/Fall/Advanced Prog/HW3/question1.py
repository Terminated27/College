...
class HashTable:
    ...
    def put(self, key, data): #EDIT
        hashvalue = self.hashfunction(key, len(self.slots)) # Encode key 
        if self.slot_content(key) is None: #check if slot has list
            self.slots[hashvalue] = OrderedList() #create linked list and put node as head
        if self.slots[hashvalue].search(key) is None: #check if data with same key exists already in list
            self.slots[hashvalue].add(key,data) # 
        else:
            temp = self.slots[hashvalue].search(key) #override data already at key location
            temp.setData(data)
